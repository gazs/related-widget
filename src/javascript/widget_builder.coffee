...
module.exports = WidgetBuilder =
  ...
  render: ->
    ...
    $('.header_input').on 'change', (e) =>
      iframe = """
          <iframe src="#{window.location.href}?posts=#{@post_ids.reverse().join(',')}&title=#{$('.header_input').val() or "Recommended stories"}" width="636" height="250" class="custom aspect recommended" id="editorial_labs_recommended_stories_widget"></iframe>
        """
      $('.result').html(iframe)
      $('.embed').val(iframe)
      # $('.link_input').select()
